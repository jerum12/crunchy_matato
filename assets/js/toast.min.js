(function (a) {
  function f(b) {
    if (!a('#toast-container').length) {
      var d = 'top-right top-left top-center bottom-right bottom-left bottom-center'
        .split(' ')
        .includes(a.toastDefaults.position)
        ? a.toastDefaults.position
        : 'top-right';
      a('body').prepend(
    ...
    var c = '',
      ...
      n = b.content,
      h = b.img,
      p = b.delay ? 'data-delay="' + b.delay + '"' : 'data-autohide="false"',
      q = '',
    ...
    switch (t) {
      ...
      case 'error':
        (e = a.toastDefaults.style.error || 'bg-danger'),
          (c = a.toastDefaults.style.error || 'text-white');
    }
    a.toastDefaults.pauseDelayOnHover &&
      b.delay &&
      ((p = 'data-autohide="false"'),
      (q = 'data-hide-after="' + (Math.floor(Date.now() / 1e3) + b.delay / 1e3) + '"'));
    c =
      '<div id="' +
      g +
      '" class="toast ' +
    ...
    h &&
      (c +=
        '<img src="' +
        h.src +
        '" class="mr-2 rounded me-2' +
        (h['class'] || '') +
        '" alt="' +
        (h.alt || 'Image') +
        '">');
    c += '<strong class="me-auto">' + u + '</strong>';
    m && (c += '<small class="text-white">' + m + '</small>');
    r &&
      (c +=
        '<button type="button" class="btn-close ml-2 mb-1 close" data-bs-dismiss="toast" data-dismiss="toast" aria-label="Close">\n                        <span aria-hidden="true" class="text-white">&times;</span>\n                    </button>');
    c += '</div>';
    n &&
      (c +=
  ...
  a.snack = function (b, d, c) {
    return f({ type: b, title: d, delay: c });
  };
  a.toast = function (b) {
    return f(b);
  };
